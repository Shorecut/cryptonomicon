  ...
  <div class="container mx-auto flex flex-col items-center bg-gray-100 p-4">
    <div class="container">
          ...
          <div class="max-w-xs">
            ...
            <div v-if="suggestions.length > 0" class="flex bg-white shadow-md p-1 rounded-md shadow-md flex-wrap">
              <span
                ...
                class="inline-flex items-center px-2 m-1 rounded-md text-xs font-medium bg-gray-300 text-gray-800 cursor-pointer hover:bg-gray-400 transition-colors"
              >
                {{suggestion}}
              </span>
            </div>
            <div v-if="ticker && tickerExists"  class="text-sm text-red-600">Такой тикер уже добавлен</div>
          </div>
        </div>
        <button
          @click="add"
      ...
      <template v-if="tickers.length > 0">
        <div>
          <button v-if="page > 1" @click="page--" class="my-4 inline-flex items-center py-2 px-4 border border-transparent shadow-sm text-sm leading-4 font-medium rounded-full text-white bg-gray-600 hover:bg-gray-700 transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500"
          >Назад
          </button>
          <button v-if="hasNextPage" @click="page++" class="my-4 inline-flex items-center py-2 px-4 border border-transparent shadow-sm text-sm leading-4 font-medium rounded-full text-white bg-gray-600 hover:bg-gray-700 transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500"
          >Вперед
          </button>
          <div>
            Фильтр:
...
export default {
  ...
  suggestions(){
    ...
    return Object.keys(this.allCoins)
      .filter(coinSymbol => {
        return coinSymbol.startsWith(upperTicker);
      }).slice(0,4)
  }
  },
  ...
  created() {
    ...
    const tickerToUpdate = localStorage.getItem("cryptonomicon-list")
    if(tickerToUpdate){
      this.tickers = JSON.parse(tickerToUpdate)
  ...
  methods: {
    ...
    subscribeToUpdate(tickerName){
      setInterval(async() => {
        const f = await fetch(`https://min-api.cryptocompare.com/data/price?fsym=${tickerName}&tsyms=USD`, {
          headers: {
            authorization: "Apikey 45ac4aadcca620a692a7148bcae113635abaf0b066d0f0595718c570c8f83af9"
          }
        });
        const data = await f.json();
        const tickerToUpdate = this.tickers.find(t => t.name === tickerName)
        if(tickerToUpdate){
          tickerToUpdate.price = data.USD > 1
            ? data.USD.toFixed(2)
    ...
    add() {
      ...
      if(this.tickerExists) return;

      const upperTicker = this.ticker.toUpperCase();
        if(!this.allCoins[upperTicker]){
          alert("hah aha")
          return;
        }
      const currentTicker = {
